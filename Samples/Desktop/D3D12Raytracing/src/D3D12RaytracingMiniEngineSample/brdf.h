...
BrdfData prepareBRDFData(float3 N, float3 L, float3 V, float3 albedo, float metalness, float roughness) {
	BrdfData data;
	...
	data.specularF0 = baseColorToSpecularF0(albedo, metalness);
	data.diffuseReflectance = baseColorToDiffuseReflectance(albedo, metalness);

	// Unpack 'perceptively linear' -> 'linear' -> 'squared' roughness
	data.roughness = max(roughness, 0.001f);
...
float3 evalCombinedBRDF(float3 N, float3 L, float3 V, float3 albedo, float metalness, float roughness) {
...
bool evalIndirectCombinedBRDF(float2 u, float3 shadingNormal, float3 geometryNormal, float3 V, float3 albedo,
	                          float metalness, float roughness, const int brdfType, 
	...
	if (brdfType == DIFFUSE_TYPE) {
		...
		const BrdfData data = prepareBRDFData(Nlocal, float3(0.0f, 0.0f, 1.0f) /* unused L vector */, Vlocal, albedo, metalness, roughness);
...
float getBrdfProbability(float3 albedo, float metalness, float roughness, float3 V, float3 shadingNormal) {
